...
The DigitalOcean VPS keeps a deployed copy of the site and exposes `origin.quizzine.org` through Nginx. It is an operational origin/rollback path; public quiz delivery is served by the Worker and R2. The Nginx configuration is kept in [`infra/nginx/quizzine.org.conf`](infra/nginx/quizzine.org.conf).
...
The site is static. From the repository root, serve it with any local static web server, for example:
